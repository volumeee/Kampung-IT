        ...
        <div class="col-lg-12 mb-3">
            <div class="card shadow">
                <div class="my-auto card-body py-3 d-sm-flex align-items-center justify-content-between">
                    <h6 class="mb-2 font-weight-bold text-primary">Form Tambah Artikel</h6>
                    <div class="ml-auto">
                        <a href="<?= base_url('admin/artikelaction'); ?>" class="btn btn-sm btn-primary shadow-sm mb-1 mb-md-0">
                                ...
                                foreach ($artikel as $s) : ?>
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <img src="<?= base_url('assets/img/artikel/') . $s['banner']; ?>" alt="banner <?= $s['judul']; ?>" class="img-thumbnail" style="width: 100px;">
                                        </td>
                                        <td><?= date('d', strtotime($s['tgl_upload'])) . ' ' . month(date('n', strtotime($s['tgl_upload'])), 'mmm') . ' ' . date('Y', strtotime($s['tgl_upload'])) . ' ' . date('H:i', strtotime($s['tgl_upload'])); ?></td>
                                        <td class="text-center"><?= number_format($s['dilihat'], 0, ",", "."); ?>x</td>
                                        <td class="text-center">
                                            <a href="<?= base_url('admin/artikelaction/') . $s['link']; ?>" class="btn btn-sm mb-1 btn-success" id="editPasien">Edit</a>
                                            <a href="" data-href="<?= base_url('admin/deleteartikel/') . $s['link']; ?>" class="btn btn-sm mb-1 btn-danger" data-toggle="modal" id="delArtikel" data-target="#deleteArtikelModal">Delete</a>
                                        </td>
                                    </tr>
                                <?php $no++;
                                endforeach; ?>
